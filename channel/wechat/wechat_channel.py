...
class WechatChannel(Channel):
    ...
    def startup(self):
        ...
        # dictdata = itchat.search_friends()
        # with open("./friends.json", 'w') as f:
        #     json.dumps(dictdata, f)
    ...
    def handle(self, msg):
        ...
        if from_user_id == other_user_id and match_prefix is not None:
            # 好友向自己发送消息
            logger.info("好友向自己发送消息 "+ json.dumps(msg, ensure_ascii=False))
            if match_prefix != '':
            ...
            # 自己给好友发送消息
            logger.info("自己给好友发送消息 "+ json.dumps(msg, ensure_ascii=False))
            str_list = content.split(match_prefix, 1)
            if len(str_list) == 2:
                content = str_list[1].strip()
            img_match_prefix = self.check_prefix(content, conf().get('image_create_prefix'))
    ...
    def _do_send(self, query, reply_user_id):
        try:
            ...
            # reply_text = query
            reply_text = super().build_reply_content(query)
            if reply_text:
